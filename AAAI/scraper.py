...
for index, issue_page in enumerate(year_ranges[year]):
    ...
    for div in soup.find_all("div"):
        ...
        if divclass is not None and "obj_article_summary" in divclass:
            ...
            title = paper_links[0].text.strip()
            titles.append(title)
            paper_url = paper_links[0]['href']
            urls.append(urls)
            pdf_url = paper_links[1]['href']
            pdf_urls.append(pdf_url)
            article_id = paper_links[0]['id'].replace("article-", "")
            ids.append(article_id)
            print(f"\t({index+1}/{total_pages})[{article_id}] Title: {title}")
            ...
            for list_item in authors_section.find_all("li"):
                ...
                a_names.append(author_name)
                a_affiliation = list_item.find("span", class_="affiliation").text.strip()
                a_aff.append(a_affiliation)
            authors.append(a_names)
            author_affiliations.append(a_aff)
            key_words_values = [kw.strip() for kw in key_words_section.find("span", class_="value").text.strip().split(",")]
            key_words.append(key_words_values)
            abstracts.append(abstract_section.text.strip().replace("\n", " "))
            how_to_cites.append(paper_soup.find("div", class_="csl-entry").text)
        else:
            continue
